...
from modules.utils import CustomLinear, build_mlp
...
class MySimpleConv_MR_test(nn.Module):
    def __init__(self, in_feats: int, out_feats: int, e_types: list, drop_rate:float=0.0,
        ...
        for e_t in self.e_types:
            self.attn_weights[e_t] = nn.Parameter(torch.Tensor([1.0]))
            nn.init.constant_(self.attn_weights[e_t], 1.0)
    ...
    def udf_edges(self, e_t: str):
        ...
        attn_weight = self.attn_weights[e_t]
        def fnc(edges):
