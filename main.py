...
from modules.utils import nll_loss, l2_regularization, nll_loss_raw, eval_pred, fixed_augmentation, f1_score, CustomLinear, build_mlp, store_model
import numpy as np
import pandas as pd
import dgl
import warnings
...
class Learnable_UCA(nn.Module):
    def __init__(self, args):
        super(Learnable_UCA, self).__init__()
        dim = args['hidden-dim']
...
def training(epoch, model, loss_func, graph, label_loader, unlabel_loader, optimizer, augmentor, args):
    model.train()
    ...
    for idx in range(num_iters):
        ...
        total_loss = args['trainable-consis-weight'] * consistency_loss + args[
            'trainable-weight-decay'] * l2_regularization(attn_drop)
...
def get_model_pred(model, graph, data_loader, sampler, args, flag):
    model.eval()
    ...
    with torch.no_grad():
        for node_idx in data_loader:
            ...
            inter_results = model(blocks, update_bn=False, return_logits=True)
...
def val_epoch(epoch, model, graph, valid_loader, test_loader, sampler, args):
    valid_dict = {}
    valid_pred, valid_target = get_model_pred(model, graph, valid_loader, sampler, args, 0)
    ...
    test_dict = {}
    test_pred, test_target = get_model_pred(model, graph, test_loader, sampler, args, 1)
...
def run_model(args):
    ...
    if args['data-set'] == 'yelp':
        _feat = pd.read_csv('data/yelp_embedding.csv', index_col=0, na_filter=False)
        full_index = pd.Index(range(graph.number_of_nodes()))
        _feat = _feat.reindex(full_index, fill_value=0)
        s_feat = torch.tensor(_feat.values, dtype=torch.float32).to(args['device'])
        graph.ndata['feature'] = torch.cat((graph.ndata['feature'], s_feat), dim=1)
    elif args['data-set'] == 'amazon':
        _feat = pd.read_csv('data/amazon_embedding.csv', index_col=0, na_filter=False)
        full_index = pd.Index(range(graph.number_of_nodes()))
        _feat = _feat.reindex(full_index, fill_value=0)
        s_feat = torch.tensor(_feat.values, dtype=torch.float32).to(args['device'])
        graph.ndata['feature'] = torch.cat((graph.ndata['feature'], s_feat), dim=1)
    elif args['data-set'] == 'tfinance':
        _feat = pd.read_csv('data/tfinance_embedding.csv', index_col=0, na_filter=False)
        full_index = pd.Index(range(graph.number_of_nodes()))
        _feat = _feat.reindex(full_index, fill_value=0)
        s_feat = torch.tensor(_feat.values, dtype=torch.float32).to(args['device'])
        graph.ndata['feature'] = torch.cat((graph.ndata['feature'], s_feat), dim=1)
    elif args['data-set'] == 'tsocial':
        _feat = pd.read_csv('data/tsocial_embedding.csv', index_col=0, na_filter=False)
        full_index = pd.Index(range(graph.number_of_nodes()))
        _feat = _feat.reindex(full_index, fill_value=0)
        s_feat = torch.tensor(_feat.values, dtype=torch.float32).to(args['device'])
        graph.ndata['feature'] = torch.cat((graph.ndata['feature'], s_feat), dim=1)
    ...
    attn_drop = Learnable_UCA(args).to(args['device'])
